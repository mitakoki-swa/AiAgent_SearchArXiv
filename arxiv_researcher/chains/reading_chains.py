...
from arxiv_researcher.models import ReadingResult, ArxivPaper
        ...
        section_indices: list[int] = chain.invoke(
            {
                ...
                "goal": goal,
                "selected_section_indices": ",".join(map(str, selected_section_indices)),
...
class CheckSufficiency:
    ...
    def __call__(self, state: dict) -> Command[Literal["set_section", "summarize", "mark_as_not_related"]]:
        goal = state.get("goal", "")
        reading_result = state.get("reading_result")
        paper = reading_result.paper
        selected_section_indices = state.get("selected_section_indices", [])
        ...
        sufficiency: Sufficiency = chain.invoke(
            {
                "title": paper.title,
                "authors": paper.authors,
                "abstract": paper.abstract,
                "sections": self.parser.get_selected_sections(markdown_text, selected_section_indices),
                "goal": goal,
                "task": reading_result.task
            }
        )
        ...
        answer = chain.invoke(
            {
                ...
                "authors": paper.authors,
                "abstract": paper.abstract,
                "context": self.parser.get_selected_sections(markdown_text, selected_section_indices),
                "goal": goal,
                "task": task
            }
        )
